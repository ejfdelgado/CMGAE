...
if (!hayValor(moduloHttp)) {
	var moduloHttp = function () {
		...
		var call = function call(url, metodo, encabezados, usarCache) {
		...
		var get = function get(url, usarCache) {
...
if (!hayValor(moduloJuegoVista)) {
	var moduloJuegoVista = function moduloJuegoVista(jElem, jElemHead, juego, moduloJuego) {
		...
		var botones = {
			'score': function score(llave) {
				return generarBoton(llave, { icono: 'fa-star', color: 'btn-default' });
			},
			'pregunta': function pregunta(llave) {
				return generarBoton(llave, { icono: 'fa-question', color: 'btn-info' });
			},
			'respuesta': function respuesta(llave) {
				return generarBoton(llave, { icono: 'fa-check', color: 'btn-primary' });
			}
		};
		...
		var modos = {
			'blanco': function blanco() {
				datos.elem.empty();
			},
			'score': function score() {
				regenerarPuntajes();
				remplazarContenido('/assets/cmgae/juego/modos/score.html', function (plantilla) {
					return $(plantilla);
				}, datos.jugadores, function (plantilla, i, llave, unJugador) {
			...
			'pregunta': function pregunta() {
				datos.elem.empty();
				remplazarContenido('/assets/cmgae/juego/modos/pregunta.html', function (plantilla) {
					plantilla = plantilla.replace('$1', datos.preguntaActual.texto);
					return $(plantilla);
				}, datos.preguntaActual.respuestas, function (plantilla, i, llave, elemento) {
		...
		var asignarModo = function asignarModo(valor) {
			datos.modo = valor;
		};
		...
		var regenerarPuntajes = function regenerarPuntajes() {
		...
		var remplazarContenido = function remplazarContenido(url, funInicio, lista, funIter, funOrdenar) {
