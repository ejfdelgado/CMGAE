	...
	var moduloArchivos = function () {
		...
		var crearBasico = function crearBasico() {
			...
			$.when(promesa).then(function (datos) {
				var temp = leerObj(datos, 'error', null);
				if (esNumero(temp) && temp != 0) {
					//Se busca crear
					var contenido = '{% extends "base.html" %}{% block content %}It works from cloud storage!{% endblock %}';
...
if (!hayValor(moduloEdicion)) {
	var moduloEdicion = function () {
		...
		var inicializar = function inicializar() {
			...
			if (typeof configureEditorsHere !== 'undefined' && esFuncion(configureEditorsHere)) {
